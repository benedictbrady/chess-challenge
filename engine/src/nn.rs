...
/// Encode a board position as a flat [1536] float32 tensor (dual perspective).
///
/// Layout: two 768-element halves, each with 12 planes × 64 squares.
///
...
/// A shared feature transformer processes each half identically, then the
/// output layers compare the two perspectives.
pub fn board_to_tensor(game: &GameState) -> Vec<f32> {
    let board = &game.board;
    let us = board.side_to_move();
    let them = !us;

    let mut tensor = vec![0.0f32; 1536];

    // First 768: STM perspective (our pieces ch 0-5, their pieces ch 6-11)
    ...
    // Last 768: NSTM perspective (their pieces ch 0-5, our pieces ch 6-11)
...
///
/// The model must accept input "board" [N, 1536] float32 and output a scalar
...
impl NnEvalBot {
    ...
    /// Each tensor in `tensors` is a flat [1536] encoding.
    ...
    fn nn_eval_batch_inner(
        ...
        let mut flat = Vec::with_capacity(n * 1536);
        for t in tensors {
            flat.extend_from_slice(t);
        }

        let input = Tensor::<f32>::from_array(([n, 1536], flat))?;
